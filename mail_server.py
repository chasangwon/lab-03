...
def save_mail(mail: List[Dict[str, str]]) -> None:
    ...
    Takes the given Python dictionary object and turns serializes it into JSON 
    string. write_text() overwrites the contents of mail_db.json, which is located
...
def add_mail(mail_entry: Dict[str, str]) -> str:
    ...
    First creates a list of mail inside mail_db.json and appends the list with 
    mail_entry, which is passed as a parameter. Then, the newly appended mail is given a 
    unique id, which is converted to a string to match the format of the dictionary.
    
    Returns:
    	str: the ID of the newly appended mail entry
    """
    mail = load_mail()
...
def delete_mail(mail_id: str) -> bool:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Stores the mail entry in a list and then goes through the list to find the entry
    with a certain ID, which has been passed as a parameter. 
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Iterates through the list containing all mail to find the mail with same mail ID, which
    is passed on as a parameter.
    
    Returns:
    	If mail with matching ID is found:
    		Python dict of mail with matching ID
    	If not found:
    		Null value
    """
    mail = load_mail()
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Iterates through all mail to find all mail sent to a certain recipient,
    which is passed as a parameter. Stores all matching mail in a Python list
    
    Returns:
    	Python list with mail with matching recipient
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """TODO: fill out this docstring (using the load_mail docstring as a guide)
    Finds all mail that were sent from a certain sender and stores all matching mail into
    a Python list.
    
    Returns
    	Python list containing mail with matching sender
    """
    mail = load_mail()
    sent = []
    for entry in mail:
...
def get_inbox_send_route(sender: str):
